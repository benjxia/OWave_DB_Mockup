...
if __name__ == "__main__":
    ...
    last_names = ["Smith", "Johnson", "Williams", "Brown", "Jones", "Garcia", 
                  "Miller", "Cooper", "Adams", "Zamora", "Nguyen" ,"Hernandez"]
    
    # Potential hardships
    causes = ["Injury/Illness", "Legal Costs/Fees", "Child Care", "Elder Care", 
              "Car Accident", "Victim of Crime", "Loss of Residency", 
              "Loss of Transportation", "Misc."]
    
    # Approval Process
    status_small = ["APPROVED", "IN-PROGRESS - APPROVAL", "DELIVERED", 
                    "DENIED - FIN FITNESS MEETING", "DENIED", "APPEALED"]
    status_big = ["APPROVED", "DELIVERED", "IN-PROGRESS - COMMITTEE", "DENIED", 
                  "APPEALED"]
    # Generate dates
    ...
    # Request amounts
    req_vals = [200, 225, 300, 350, 475, 790, 860, 540, 550, 999, 800, 100, 150, 
                450, 400, 600, 650, 900, 120, 110, 105, 65, 50, 1500, 2000, 
                1200, 1400, 2000, 2400, 2500]
    
    ticket_num = 100000
    
    with open("data.csv2", 'w', newline='') as csvfile:
        builder = csv.writer(csvfile)
        builder.writerow(["ticket", "first_name", "last_name", 
                          "apply_timestamp", "last_update", "cause",
                          "requested", "status", "given"])
        # Write each sample row
        for i in range(1000):
            # Generate row
            ticket_num += random.randint(0,1000)
            curr_time = curr_time + datetime.timedelta(
                                                days = random.randint(0, 10), 
                                                seconds = random.randint(0,60), 
                                                hours = random.randint(0,24), 
                                                minutes = random.randint(0, 60))
            last_update = curr_time + datetime.timedelta(
            ...
            builder.writerow([ticket_num, fname, lname, ctime_str, lutime_str, 
                              ccause, req_amt, cstatus, req_gvn])
